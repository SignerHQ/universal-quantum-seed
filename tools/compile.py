...
def get_variants(word):
    ...
    Returns normalized strings in deterministic source-first order. For scripts
    with optional diacritics, the stripped form follows the original form.
    Stable ordering is part of reproducible ``words.py`` generation.
    """
    nw = normalize(word)
    variants = [nw]
...
def compile_lookup():
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8", newline="\n") as f:
        f.write("# Copyright (c) 2026 Lock.com — PolyForm Shield License 1.0.0\n")
    ...
    # Keep the package's hardcoded integrity trust root synchronized with the
    # reviewed generated artifact. The sidecar alone is not a trust anchor.
    trust_path = os.path.join(PROJECT_DIR, "__init__.py")
    with open(trust_path, "r", encoding="utf-8-sig") as f:
        trust_source = f.read().replace("\r\n", "\n")
    trust_pattern = re.compile(
        r'(_TRUSTED_WORDLIST_SHA256\s*=\s*\(\s*\n\s*")'
        r'[0-9a-f]{64}'
        r'("\s*\n\))'
    )
    trust_source, replacements = trust_pattern.subn(
        rf"\g<1>{digest}\g<2>", trust_source
    )
    if replacements != 1:
        raise RuntimeError(
            "could not locate exactly one _TRUSTED_WORDLIST_SHA256 trust root"
        )
    with open(trust_path, "w", encoding="utf-8", newline="\n") as f:
        f.write(trust_source)
    print(f"  Updated integrity trust root in {trust_path}")
